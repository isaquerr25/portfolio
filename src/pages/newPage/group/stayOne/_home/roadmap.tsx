...
import { AiFillFlag, AiFillLinkedin, AiOutlineFlag } from 'react-icons/ai';
	...
	const Points: RoadMapPoint[] = [
		{
			...
			points: ['Before new year'],
		},
		{
			name: 'Season #1',
			date: new Date('1-1-2022'),
			points: ['MVP version', 'Design Characters, Rooms'],
		},
		{
			name: 'Season #2',
			date: new Date('4-1-2022'),
			points: ['Alpha version'],
		},
	];
			...
			<div className='absolute z-10 top-[5vh] left-[5vw] backdrop-blur-xl bg-white/100 rounded-full w-[150px] h-[150px] shadow-lg'>
				<div className='absolute top-[12px] left-[67px]'>
					{'ROAD MAP    ROAD MAP'.split('').map((text, index) => (
						<span
							key={index}
							className='h-[130px] w-[15px] absolute top-0 left-0'
				...
				<Icon
					className='top-[45px] left-[47px] absolute text-pink-600 '
					h='auto'
					w='4rem'
					as={AiOutlineFlag}
			...
			<div className='lg:flex flex-nowrap pl-0 pt-16 sm:pt-0 w-[100vw] sm:pl-[15vw] m:w-[80vw]'>
				{Points.map((item, index) => (
					<div
						...
						style={{
							top: `calc(40px * ${index})`,
							backgroundColor: GetRGBColor(item.date),
						}}
					...
					>
						<div className='flex flex-nowrap items-center p-5 bg-white/60 backdrop-blur-lg rounded-lg'>
							<Icon
								className='top-[45px] left-[47px]  text-pink-600 '
								h='auto'
								w='4rem'
								as={AiFillFlag}
							/>
						...
						<strong className='mt-4 mb-2 text-black/80'>
							List of changes:{' '}
						</strong>
						<ul className='text-sm text-black/60'>
							{item.points.map((point, jndex) => (
								<li key={jndex}>{point}</li>
							))}
						</ul>
					</div>
				))}
