...
export const MarketItem = (props: Props) => {
	...
	return (
		...
		>
			<span
				style={{ backgroundColor: GetRGBColor(item.price) }}
				className='text-white px-2 py-1 text-xs absolute top-2 right-8 z-0 rounded-md'
			>
				{item.price} <sub>TOWL</sub>
			</span>
			<img
			...
			/>
			<a
				className='flex items-center justify-center rounded-full w-[45px] h-[45px] text-white bg-violet-500 drop-shadow-[0_4px_#7c3aed] absolute top-[185px] right-10 z-0'
				href={`/market/${item.id}`}
			>
			...
			<span className='ml-2 text-gray-500'>
				Sales start: {GetDate(item.start)}
			</span>
			<p className='ml-2 text-slate-600'>{item.description}</p>
			<div className='flex flex-wrap items-center mt-4'>
				<span
					style={{
						color: GetRGBColor(item.price),
						backgroundColor: GetRGBAColor(item.price),
					}}
					className='mr-2 mb-2 flex flex-nowrap items-center rounded-md w-fit px-2 py-1 text-xs'
				>
					<i className='material-icons text-sm mr-2'>people</i>
					<span>
						{item.offers} Person{item.offers > 2 ? 's' : ''} offered
					</span>
				</span>
				{item.keywords.map((keyword, index) => (
					<span
						key={index}
						className='mr-2 mb-2 px-[12px] py-[4px] h-fit rounded-md bg-slate-100 text-slate-600 text-sm'
					>
						{keyword}
					</span>
				))}
